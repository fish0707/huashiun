...
const BRANDS = [
  'Apple', 'Samsung', 'ASUS', 'Sony', 'Google Pixel',
  'Vivo', 'OPPO', 'Xiaomi', 'Nokia', '其他'
]
...
export default function NewPhoneFormPage() {
  ...
  useEffect(() => {
    if (isNew) return
    supabase.from('new_phones').select('*').eq('id', id).single().then(({ data }) => {
      if (data) {
  ...
  const handleCoverUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const url = await uploadFile(file, 'new-covers')
  ...
  const handlePhotosUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const urls = await Promise.all(files.map(f => uploadFile(f, 'new-photos')))
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (isNew) await supabase.from('new_phones').insert({ ...payload, views: 0 })
    else await supabase.from('new_phones').update(payload).eq('id', id)
          ...
          <div className="bg-white border border-border rounded-2xl p-5 space-y-4">
            ...
            {form.brand === '其他' && (
              <div>
                ...
                <input required value={form.customBrand}
                  ...
                  className="w-full px-4 py-2.5 text-sm border border-border rounded-xl outline-none focus:border-accent"
                  placeholder="例如：Realme、Motorola、HTC..." />
              </div>
            )}
            ...
            <div>
              ...
              <input value={form.storage}
                ...
                className="w-full px-4 py-2.5 text-sm border border-border rounded-xl outline-none focus:border-accent"
                placeholder="例如：128GB、256GB、512GB、1TB" />
            </div>
            ...
            <div>
              ...
              <textarea rows={3} value={form.description}
                ...
                placeholder="台灣公司貨，盒裝配件齊全，原廠一年保固..." />
            </div>
            ...
            <div className="flex flex-col gap-3 pt-1">
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <div>
                  <p className="text-sm font-medium">現貨供應中</p>
                  <p className="text-xs text-muted">勾選後商品列表會顯示綠色現貨標示</p>
                </div>
              </label>
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <div>
                  <p className="text-sm font-medium">上架中</p>
                  <p className="text-xs text-muted">取消勾選後商品不會顯示在前台</p>
                </div>
              </label>
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={form.is_featured}
                  ...
                  className="w-4 h-4 accent-[#C8A96E]" />
                <div>
                  <p className="text-sm font-medium">設為熱門推薦</p>
                  <p className="text-xs text-muted">顯示在首頁熱門新機區塊</p>
                </div>
              </label>
            </div>
          </div>
